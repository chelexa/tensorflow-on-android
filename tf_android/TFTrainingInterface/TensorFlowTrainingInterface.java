...
public class TensorFlowInferenceInterface {
  private static final String TAG = "TensorFlowTrainingInterface";
  ...
  public TensorFlowInferenceInterface(AssetManager assetManager, String model) {
    Log.i(TAG, "Checking if TensorFlow native methods already loaded");
    try {
   ...
   */
  public void run(String[] outputNames, String[] targetNames) {
    run(outputNames, targetNames, false);
  }

  /**
  * Added in TensorFlowTrainingInterface
  */
  public void run(String[] outputsNames){
      run(outputsNames, new String[]{}, false);
  }
  /**
  * Added in TensorFlowTrainingInterface
  */
  public void run(String[] outputsNames, boolean enableStats){
      run(outputsNames, new String[]{}, enableStats);
  }
   ...
   * @param outputNames A list of output nodes which should be filled by the inference pass.
   */
  public void run(String[] outputNames, String[] targetNames, boolean enableStats) {
    ...
    /* Added in TensorFlowTrainingInterface to allow running operations with no output */
    // Add targets.
    for (String t : targetNames) {
      this.targetNames.add(t);
      runner.addTarget(t);
    }
  ...
  public void close() {
    closeFeeds();
    closeFetches();
    closeTargets(); // Added in TensorFlowTrainingInterface
    sess.close();
    g.close();
    if (runStats != null) {
      runStats.close();
  ...
  private void addFeed(String inputName, Tensor t) {
    // The string format accepted by TensorFlowTrainingInterface is node_name[:output_index].
